...
//! `sdd/forma-core-object-schema/spec` #1623).

use forma_core::error::{FieldPath, Segment};
use forma_core::prelude::*;
use forma_core::schema::{DynSchema, Schema, ShapeKind as K};
use forma_core::value::{Object, ToValue, Value};
...
fn os1_any_child_composition_validates() {
    let schema = object()
        ...
        .field("n", forma_core::coerce::coerced::<u32>())
        .field("sub", object().field("b", forma_core::types::bool()));
    let input = obj(&[
        ("s", Value::from("abc")),
        ("n", Value::from("7")),
...
fn os3_three_level_nesting_succeeds_preserving_all_levels() {
    let schema = object().field(
        "a",
        object().field("b", object().field("c", forma_core::types::bool())),
    );
    let input = obj(&[(
        "a",
        Value::Object(obj(&[(
...
fn os3_nested_absent_key_reports_required_at_depth() {
    let schema = object().field(
        "outer",
        object().field("qty", forma_core::coerce::coerced::<u32>()),
    );
