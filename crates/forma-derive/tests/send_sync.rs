...
//! the forma-core SS-lineage probe style.
//!
...
#[derive(FormSchema)]
struct Pinned {
    #[form(schema = ::forma_core::types::string().min(2).refine(|s: &str| s != PINNED_SENTINEL))]
    guarded: String,
    #[form(schema = ::forma_core::types::string().trim().refine(|s: &str| !s.is_empty()))]
    boxed_rule: String,
    child: Leaf,
}
