//! Opt-in `#[derive(FormSchema)]` for [`forma-core`] schemas.
//!
//! `forma-core` is builder-first and macro-free; this crate is the optional
...
//! whose single composed representation is a real `::forma_core::ObjectSchema`
...
//! assert!(parsed.is_ok());
//!
//! // Erased view over a raw Value tree (what forma-signals/forma-ui consume).
...
//!
//! - `pub struct XSchema` owning its composed `::forma_core::ObjectSchema`,
...
//! - `impl AsRef<::forma_core::ObjectSchema> for XSchema`;
...
//!
//! All generated code references core items via absolute `::forma_core::..`
//! paths (proc-macros have no `$crate` escape hatch). Renaming or shadowing
//! the `forma-core` package is therefore a breaking change for derive users.
