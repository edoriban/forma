...
//! composed `::forma_core::ObjectSchema`, with both views (`Schema`,
//! `DynSchema`) delegating to it (SC-9), plus the nesting link
//! (`FormSchema for X`) and the value bridge (`FormBridge for X`).
//!
//! Every generated path is absolute `::forma_core::...` (CR-4). Per-field
...
//! - [`::forma_core::form::FormSchema`] — supplies the composed child schema
//!   (`Nested::new(<T as FormSchema>::form_schema())`);
//! - [`::forma_core::form::FormBridge`] — supplies the typed-parse bridging
...
fn default_child(plan: &FieldPlan) -> TokenStream {
    ...
    match single_segment_ident(ty).as_deref() {
        Some("String") => quote_spanned! { span=> ::forma_core::types::string() },
        Some("bool") => quote_spanned! { span=> ::forma_core::types::bool() },
        Some("i8" | "i16" | "i32" | "i64" | "u8" | "u16" | "u32" | "f32" | "f64") => {
            quote_spanned! { span=> ::forma_core::coerce::coerced::<#ty>() }
        }
        _ => quote_spanned! { span=>
            ::forma_core::schema::Nested::new(<#ty as ::forma_core::form::FormSchema>::form_schema())
        },
    }
}
...
pub(crate) fn expand(input: &DeriveInput) -> Result<TokenStream, syn::Error> {
    ...
    for p in &active {
        ...
        let insert = if is_numeric_matrix(&p.ty) {
            ...
            let ty = &p.ty;
            quote_spanned! { p.ident.span()=>
                <#ty as ::forma_core::form::FormBridge>::to_form_value(&self.#ident)
            }
        };
        to_form_inserts.push(quote_spanned! { p.ident.span()=>{
            ::forma_core::value::Object::insert(&mut __obj, #key, #insert);
        }});
    }
    ...
    for p in &active {
        ...
        from_validated_fields.push(quote_spanned! { p.ident.span()=>
            #ident: __obj
                .get(#key)
                .and_then(<#ty as ::forma_core::form::FormBridge>::from_validated)?,
        });
    }
    ...
    let (from_validated_param, from_validated_body) = if skipped.is_empty() {
        (
            ...
            quote_spanned! { Span::call_site()=>
                let ::forma_core::value::Value::Object(__obj) = v else {
    ...
    let expanded = quote_spanned! { Span::call_site()=>
        ...
        #vis struct #companion {
            object: ::forma_core::types::ObjectSchema,
        }
        ...
        impl #companion {
            ...
            pub fn new() -> Self {
                Self {
                    object: ::forma_core::types::object()
                        #(#field_registrations)*
                }
            }
            ...
            pub fn field_meta(&self, key: &str)
                -> ::std::option::Option<&::forma_core::schema::FieldMeta> {
        ...
        #[automatically_derived]
        impl ::std::convert::AsRef<::forma_core::types::ObjectSchema> for #companion {
            fn as_ref(&self) -> &::forma_core::types::ObjectSchema {
                &self.object
            }
        }
        ...
        #[automatically_derived]
        impl ::forma_core::schema::Schema for #companion {
            ...
            fn parse(
                &self,
                input: &#struct_ident,
            ) -> ::std::result::Result<#struct_ident, ::forma_core::error::FormaError> {
                let ::forma_core::value::Value::Object(__obj) =
                    <#struct_ident as ::forma_core::form::FormBridge>::to_form_value(input)
                else {
                    ::std::unreachable!("derived `to_form_value` always returns `Value::Object`")
                };
                let __validated = ::forma_core::schema::Schema::parse(&self.object, &__obj)?;
        ...
        #[automatically_derived]
        impl ::forma_core::schema::DynSchema for #companion {
            fn validate_value(
                &self,
                v: &::forma_core::value::Value,
            ) -> ::std::vec::Vec<::forma_core::error::FormaIssue> {
                ::forma_core::schema::DynSchema::validate_value(&self.object, v)
            }

            fn shape(&self) -> &::forma_core::schema::ShapeNode {
                ::forma_core::schema::DynSchema::shape(&self.object)
            }

            fn metadata(&self) -> &::forma_core::schema::FieldMeta {
                ::forma_core::schema::DynSchema::metadata(&self.object)
            }
        }

        #[automatically_derived]
        impl ::forma_core::form::FormSchema for #struct_ident {
        ...
        #[automatically_derived]
        impl ::forma_core::form::FormBridge for #struct_ident {
            fn to_form_value(&self) -> ::forma_core::value::Value {
                let mut __obj = ::forma_core::value::Object::new();
                #(#to_form_inserts)*
                ::forma_core::value::Value::Object(__obj)
            }

            fn from_validated(#from_validated_param: &::forma_core::value::Value) -> ::std::option::Option<Self> {
                #from_validated_body
            }
        }
    };
...
fn reconstruction_field(p: &FieldPlan) -> TokenStream {
    ...
    let ty = &p.ty;
    quote_spanned! { p.ident.span()=>
        #ident: match ::forma_core::value::Object::get(&__validated, #key)
            .and_then(<#ty as ::forma_core::form::FormBridge>::from_validated)
        {
            ::std::option::Option::Some(__field) => __field,
            ::std::option::Option::None => {
                return ::std::result::Result::Err(::forma_core::error::FormaError {
                    issues: ::std::vec![::forma_core::error::FormaIssue {
                        path: ::forma_core::error::FieldPath::key(#key),
                        code: ::forma_core::error::IssueCode::TypeMismatch,
