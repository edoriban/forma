...
//! single-import example shape using NO direct `formars_signals` import.
//! Schema builders come from `formars-core` (the validation layer), not from
//! formars-signals — that edge is unchanged from the base design.
...
fn inventory_audit_references_every_public_item() {
    ...
    let hook: fn(ValidateOn) -> UseForm = formars_ui::use_form;
    let expect_hook: fn() -> FormController = formars_ui::use_form_controller;
    let try_hook: fn() -> Option<FormController> = formars_ui::try_form_controller;
...
/// `formars_signals` import anywhere in this file.
#[test]
