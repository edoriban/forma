...
Leptos **0.8** companion crate for [`forma-signals`](../forma-signals): the
`use_form` hook, the `<Form>` submit wrapper, and the `<TextField>` field
binding. All reactive state lives in `forma-signals`; this crate adds only
...
```rust
use forma_core::prelude::*; // schema builders live in the validation layer
use forma_ui::*;
...
in the feature graph. That is EXPECTED and harmless: `forma-signals` code
paths never schedule effects (`FS-DEP-2` intact); only `<TextField>`'s
display effect uses the always-present module. Verify with
`cargo tree -e features -p forma-ui`: reactive_graph feature edges come from
the leptos subtree only, never from forma-signals.
