...
use formars_core::schema::{DynSchema, Schema};
...
fn sig(issues: &[formars_core::error::FormaIssue]) -> Vec<(String, IssueCode)> {
    issues
        .iter()
...
fn dp1_flat_field_views_agree_on_every_typed_representable_input() {
    ...
    for user in &users {
        let wire = <User as ::formars_core::form::FormBridge>::to_form_value(user);
        let erased_ok =
...
fn dp1_nested_failure_agrees() {
    ...
    let wire = <Outer as ::formars_core::form::FormBridge>::to_form_value(&Outer {
        inner: Inner { qty: 7 },
    });
